    ...
    <div class="flex flex-row gap-3 w-full mb-[10px] sm:mb-0">
        <div class="flex flex-row justify-between w-full sm:gap-4 gap-2 items-center">
            <div class="flex flex-row sm:gap-4 gap-2 items-center" >
                ...
                {{--<h1 class="text-xl xl:text-lg lg:text-base md:text-sm  font-bold  font-blue"> {{ request()->segment(1) }} Player News</h1>--}}
            ...
            <div class="nhl-pn-container-header container-header">
                ...
                <div class="nhl-pn-container-slider container-slider">
                    <div class="nhl-pn-slider-wrapper slider-wrapper">
                        <!-- Slider controls -->
                        <button type="button" id="prev-slide" class="nhl-pn-slide-button home-pn-slide-button slide-button absolute start-0 z-30   items-center justify-center  cursor-pointer group focus:outline-none" data-carousel-prev>
                        ...
                        <div  class="
                        nhl-pn-image-list
                                {{--{{ (request()->segment(1) == 'nba') ? 'nhl-pn-image-list' : '' }}--}}
                        {{--{{ (request()->segment(1) == 'nhl') ? 'nhl-pn-image-list' : '' }}--}}
                        {{--{{ (request()->segment(1) == 'mlb') ? 'mlb-pn-image-list' : '' }}--}}
                                home-pn-image-list image-list">
                            @php
                                $pn_count = 0;
                                $pn_bg_color = '';
                                $pn_type_icon = ''
                            ...
                            @foreach($player_news->data as $key=>$val)
                                ...
                                @endphp
                                {{--{{ ($pn_count  > 6) ? 'hidden' : '' }}--}}
                                @if($player_news->data->player->position->slug == 'offensive-line')
                                    <div class="
                                    {{ (request()->segment(1) == 'nba') ? 'nba-image-item' : '' }}
                                    {{ (request()->segment(1) == 'nhl') ? 'nhl-pn-image-item' : '' }}
                                    {{ (request()->segment(1) == 'mlb') ? 'mlb-image-item' : '' }}
                                    {{ (request()->segment(1) == 'nfl') ? 'nfl-image-item' : '' }}
                                    {{ (request()->segment(1) == 'epl') ? 'epl-image-item' : '' }}
                                            home-pn-image-item image-item" >
                                        ...
                                        <div class=" card-wd bg-white rounded-[13px]">
                                            <div class="flex flex-col gap-4 items-center justify-center relative">
                                                ...
                                                @elseif($val->type->slug == 'doubtful' || $val->type->slug == 'questionable')
                                                    @php
                                                        $pn_bg_color = 'bg-[#FE6601]';
                                                        $pn_type_icon ='';
                                                    @endphp
                                                @elseif($val->type->slug == 'game-time-decision')
                                                    @php
                                                        $pn_bg_color = 'bg-[#000000]';
                                                        $pn_type_icon ='';
                                                    @endphp
                                                @else
                                                    @php
                                                        $pn_bg_color = 'bg-[#38B6FF]';
                                                        $pn_type_icon ='';
                                                    @endphp
                                                @endif

                                                <div class="flex flex-row justify-center gap-3 {{ $pn_bg_color }} p-2 text-white items-center w-full rounded-b-[13px]">
                                                    @if(!empty($pn_type_icon))
                                                        <img class="w-[15px] h-[15px]" src="{{ asset($pn_type_icon) }}">
                                                    @endif
                        ...
                        <button type="button" id="next-slide" class="nhl-pn-slide-button slide-button absolute  end-0 z-30  items-center justify-center  cursor-pointer group focus:outline-none right-0"  data-carousel-next>
                            <span class="inline-flex items-center justify-center w-11 h-11 rounded-full bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-none">
                                <svg class="w-4 h-4 text-white dark:text-gray-800 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
