            ...
            <div class="nhl-pn-container-header container-header">
                ...
                <div class="nhl-pn-container-slider container-slider">
                    <div class="nhl-pn-slider-wrapper slider-wrapper">
                        <!-- Slider controls -->
                        <button type="button" id="prev-slide-wide-receiver"  rel="wide-receiver"   class="hidden nhl-pn-slide-button home-pn-slide-button slide-button absolute start-0 z-30   items-center justify-center  cursor-pointer group focus:outline-none" data-carousel-prev>
                        ...
                        <div  class="nhl-pn-image-list home-pn-image-list image-list-wide-receiver image-list" rel="wide-receiver">
                            @php
                                $wide_receiver_count = 0;
                                $wide_receiver_bg_color = '';
                                $wide_receiver_type_icon = ''
                            @endphp
                            @foreach($wide_receiver as $key=>$val)
                                @php
                                    $wide_receiver_count++
                                @endphp
{{--                                {{ ($wide_receiver_count  > 6) ? 'hidden' : '' }}--}}

                                <div class="nfl-image-item home-pn-image-item image-item wide-receiver" >
                                        ...
                                        <div class=" card-wd bg-white rounded-[13px]">
                                            <div class="flex flex-col gap-4 items-center justify-center relative">
                                                ...
                                                @if($val->type->slug == 'injury' || $val->type->slug == 'send-down' || $val->type->slug == 'suspension' || $val->type->slug == 'ruled-out' || $val->type->slug == 'healthy-scratch')
                                                    @php
                                                        $wide_receiver_bg_color = 'bg-[#F10000]';
                                                    @endphp
                                                ...
                                                @elseif($val->type->slug == 'doubtful' || $val->type->slug == 'questionable')
                                                    @php
                                                        $wide_receiver_bg_color = 'bg-[#FE6601]';
                                                        $wide_receiver_type_icon ='';
                                                    @endphp
                                                @elseif($val->type->slug == 'game-time-decision')
                                                    @php
                                                        $wide_receiver_bg_color = 'bg-[#000000]';
                                                        $wide_receiver_type_icon ='';
                                                    @endphp
                                                @else
                                                    @php
                                                        $wide_receiver_bg_color = 'bg-[#38B6FF]';
                                                        $wide_receiver_type_icon ='';
                                                    @endphp
                                                @endif

                                                <div class="flex flex-row justify-center gap-3 {{ $wide_receiver_bg_color }} p-2 text-white items-center w-full rounded-b-[13px]">
                                                    @if(!empty($wide_receiver_type_icon))
                                                        <img class="w-[15px] h-[15px]" src="{{ asset($wide_receiver_type_icon) }}">
                                                    @endif
                        ...
                        <button type="button"  id="next-slide-wide-receiver"  rel="wide-receiver" class="nhl-pn-slide-button slide-button absolute  end-0 z-30  items-center justify-center  cursor-pointer group focus:outline-none right-0"  data-carousel-next>
                            <span class="inline-flex items-center justify-center w-11 h-11 rounded-full bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-none">
                                <svg class="w-4 h-4 text-white dark:text-gray-800 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
