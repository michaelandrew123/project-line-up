<div class="flex flex-col w-full">
    <div class="flex flex-row gap-3 w-full mb-[10px] sm:mb-0">
        <div class="flex flex-row justify-between items-center w-full items-center">
            <div class="flex flex-row sm:gap-4 gap-2 items-center">
                ...
                <h1 class="font-blue text-xl xl:text-lg lg:text-base md:text-sm  font-bold ">NFL</h1>
            </div>
            <div class=" flex justify-center items-center">
                <a href="./nfl/home" class="font-blue font-bold text-sm">View All </a>
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-col w-full xl:gap-5 lg:gap-3 md:gap-2  ">
        ...
        <div class=" w-full  ">
            <div class="container-header nfl-container-header">

                <div class="container-slider nfl-container-slider">
                    <div class="slider-wrapper nfl-slider-wrapper">
                        <!-- Slider controls -->
                        <button type="button" id="prev-slide" class=" slide-button nfl-slide-button absolute start-0 z-30   items-center justify-center  cursor-pointer group focus:outline-none" data-carousel-prev>
                        ...
                        <div  class="image-list nfl-image-list">
                            @php
                                $nfl_count = 0;
                                $nfl_bg_color = '';
                                $nfl_type_icon = ''
                            @endphp
                            @foreach($nfl_results->data as $key=>$val)
                                @php
                                    $nfl_count++
                                @endphp
                                {{--{{ ($nfl_count  > 6) ? 'hidden' : '' }}--}}

                                <div class="nfl-image-item image-item" >
                                    ...
                                    <div class="card-wd bg-white rounded-[13px] ">
                                        <div class="flex flex-col gap-4 items-center justify-center relative">
                                            ...
                                            @elseif($val->type->slug == 'confirmed' || $val->type->slug == 'available' || $val->type->slug == 'call-up' || $val->type->slug == 'will-play')
                                                @php
                                                    $nfl_type_icon ='';
                                                    $nfl_bg_color = 'bg-[#15D869]';
                                                @endphp
                                            @elseif($val->type->slug == 'expected' || $val->type->slug == 'probable')
                                                @php
                                                    $nfl_type_icon ='';
                                                    $nfl_bg_color = 'bg-[#FFCC00]';
                                                @endphp
                                            @elseif($val->type->slug == 'doubtful' || $val->type->slug == 'questionable')
                                                @php
                                                    $nfl_bg_color = 'bg-[#FE6601]';
                                                    $nfl_type_icon ='';
                                                @endphp
                                            @elseif($val->type->slug == 'game-time-decision')
                                                @php
                                                    $nfl_bg_color = 'bg-[#000000]';
                                                    $nfl_type_icon ='';
                                                @endphp
                                            @else
                                                @php
                                                    $nfl_bg_color = 'bg-[#38B6FF]';
                                                    $nfl_type_icon ='';
                                                @endphp
                                            @endif
                                            <div class="flex flex-row justify-center gap-3 {{ $nfl_bg_color }} p-2 text-white items-center w-full rounded-b-[13px] ">
                                    ...
                                    <div class="">
                                        ...
                                        @if(!empty($val->type->slug) && $val->type->slug == 'lineup-update')
                                            ...
                                            <a href="/nfl/depth-chart/{{  ( !empty($val->team->slug) ? $val->team->slug : '')     }}" class="card-wd flex flex-row items-center gap-2 justify-center  p-2 rounded-[13px] border border-[#d9d9d9] bg-white cursor-pointer  "  >
                                        ...
                                        @else
                                            <div class="card-wd flex flex-row items-center gap-2 justify-center  p-2 rounded-[13px] border border-[#d9d9d9] bg-white cursor-pointer nfl-view-news " rel="modal-{{$val->id}}" >
                                                <img class="w-[15px] h-[15px]" src="{{ asset('/images/home-page/plus.png') }}">
                                                <h1 class="text-base ">
                                                    view news
                                                </h1>
                                        ...
                                        <div class="h-full nfl-view-news-desc  hidden" id="nfl-{{$val->id}}">
                                ...
                                {{--<div class="absolute">--}}
                                    {{--<div class="relative z-30 hidden modalNfl {{$val->id}} ease-out duration-300" id="modalNfl-{{$val->id}}" aria-labelledby="modal-title" role="dialog" aria-modal="true">--}}
                                        {{--<!----}}
                                        ...
                                        {{--<div class="fixed inset-0 z-10 w-screen overflow-y-auto">--}}
                                            {{--<div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">--}}
                                                ...
                                                {{--<div class="modal-center relative transform overflow-hidden rounded-lg text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">--}}
                                                    ...
                                                    {{--<div class="close-modal cursor-pointer" rel="modalNfl-{{$val->id}}"></div>--}}
                        ...
                        <button type="button" id="next-slide" class="slide-button nfl-slide-button  absolute  end-0 z-30  items-center justify-center  cursor-pointer group focus:outline-none right-0"  data-carousel-next>
                            <span class="inline-flex items-center justify-center w-11 h-11 rounded-full bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-none">
                                <svg class="w-4 h-4 text-white dark:text-gray-800 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
                    ...
                    <div class="nfl-slider-scrollbar" style="visibility: hidden">
                        <div class="scrollbar-track">
                            <div class="nfl-scrollbar-thumb scrollbar-thumb"></div>
                        </div>
                    </div>
