<div class="flex flex-col w-full">
    <div class="flex flex-row gap-3 w-full mb-[10px] sm:mb-0">
        <div class="flex flex-row justify-between items-center w-full items-center">
            <div class="flex flex-row  sm:gap-4 gap-2  items-center">
                ...
                <h1 class="font-blue text-xl xl:text-lg lg:text-base md:text-sm  font-bold ">MLB</h1>
            </div>
            <div class=" flex justify-center items-center">
                <a href="./mlb/home" class="font-blue font-bold text-sm">View All </a>
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-col w-full xl:gap-5 lg:gap-3 md:gap-2 ">
        ...
        <div class=" w-full">
            <div class="container-header mlb-container-header">

                <div class="container-slider mlb-container-slider">
                    <div class="slider-wrapper mlb-slider-wrapper">
                        <!-- Slider controls -->
                        <button type="button" id="prev-slide" class=" slide-button mlb-slide-button absolute start-0 z-30   items-center justify-center  cursor-pointer group focus:outline-none" data-carousel-prev>
                        ...
                        <div  class="image-list mlb-image-list">
                            @php
                                $mlb_count = 0;
                                $mlb_bg_color = '';
                                $mlb_type_icon = ''
                            @endphp
                            @foreach($mlb_results->data as $key=>$val)
                                @php
                                    $mlb_count++
                                @endphp
                                {{--{{ ($mlb_count  > 6) ? 'hidden' : '' }}--}}

                                <div class="mlb-image-item image-item" >
                                    ...
                                    <div class=" card-wd bg-white rounded-[13px]  ">
                                        <div class="flex flex-col gap-4 items-center justify-center relative">
                                            ...
                                            @elseif($val->type->slug == 'confirmed' || $val->type->slug == 'available' || $val->type->slug == 'call-up' || $val->type->slug == 'will-play')
                                                @php
                                                    $mlb_type_icon ='';
                                                    $mlb_bg_color = 'bg-[#15D869]';
                                                @endphp
                                            @elseif($val->type->slug == 'expected' || $val->type->slug == 'probable')
                                                @php
                                                    $mlb_type_icon ='';
                                                    $mlb_bg_color = 'bg-[#FFCC00]';
                                                @endphp
                                            @elseif($val->type->slug == 'doubtful' || $val->type->slug == 'questionable')
                                                @php
                                                    $mlb_bg_color = 'bg-[#FE6601]';
                                                    $mlb_type_icon ='';
                                                @endphp
                                            @elseif($val->type->slug == 'game-time-decision')
                                                @php
                                                    $mlb_bg_color = 'bg-[#000000]';
                                                    $mlb_type_icon ='';
                                                @endphp
                                            @else
                                                @php
                                                    $mlb_bg_color = 'bg-[#38B6FF]';
                                                    $mlb_type_icon ='';
                                                @endphp
                                            @endif
                                            <div class="flex flex-row justify-center gap-3 {{ $mlb_bg_color }} p-2 text-white items-center w-full rounded-b-[13px] ">
                                                {{--<img class="w-[15px] h-[15px]" src="{{ asset('/images/menu-icon/injury icon.png') }}">--}}

                                                @if(!empty($mlb_type_icon))
                                                    <img class="w-[15px] h-[15px]" src="{{ asset($mlb_type_icon) }}">
                                                @endif
                                    ...
                                    <div class="">
                                        ...
                                        @if(!empty($val->type->slug) && $val->type->slug == 'lineup-update')
                                            ...
                                            <a href="/mlb/starting-lineups/{{  ( !empty($val->team->slug) ? $val->team->slug : '')     }}" class="card-wd flex flex-row items-center gap-2 justify-center  p-2 rounded-[13px] border border-[#d9d9d9] bg-white cursor-pointer  "  >
                                        ...
                                        @else
                                            <div class="card-wd flex flex-row items-center gap-2 justify-center  p-2 rounded-[13px] border border-[#d9d9d9] bg-white cursor-pointer mlb-view-news " rel="modal-{{$val->id}}" >
                                                <img class="w-[15px] h-[15px]" src="{{ asset('/images/home-page/plus.png') }}">
                                                <h1 class="text-base ">
                                                    view news
                                                </h1>
                                        ...
                                        <div class="h-full mlb-view-news-desc  hidden" id="mlb-{{$val->id}}">
                                ...
                                {{--<div class="absolute">--}}
                                    {{--<div class="relative z-30 hidden modalMlb {{$val->id}} ease-out duration-300" id="modalMlb-{{$val->id}}" aria-labelledby="modal-title" role="dialog" aria-modal="true">--}}
                                        ...
                                        {{--<div class="fixed inset-0 z-10 w-screen overflow-y-auto">--}}
                                            {{--<div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">--}}
                                                ...
                                                {{--<div class="modal-center relative transform overflow-hidden rounded-lg text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">--}}
                                                    ...
                                                    {{--<div class="close-modal cursor-pointer" rel="modalMlb-{{$val->id}}"></div>--}}
                        ...
                        <button type="button" id="next-slide" class="slide-button mlb-slide-button  absolute  end-0 z-30  items-center justify-center  cursor-pointer group focus:outline-none right-0"  data-carousel-next>
                            <span class="inline-flex items-center justify-center w-11 h-11 rounded-full bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-none">
                                <svg class="w-4 h-4 text-white dark:text-gray-800 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
                    ...
                    <div class="mlb-slider-scrollbar" style="visibility: hidden">
                        <div class="scrollbar-track">
                            <div class="mlb-scrollbar-thumb scrollbar-thumb"></div>
                        </div>
                    </div>
